...
def ensemble_probs_from_rules(rules: List[Dict[str, Any]]) -> tuple[Dict[str, float], Dict[str, float]]:
    ...
    for rule in rules:
        ...
        weighted.append((probs, w))
    if not weighted:
        return {"REJECT": 0.34, "BREAK_FAIL": 0.33, "ACCEPT_CONTINUE": 0.33}, {}
    return blend_probs(weighted), weights


def ensemble_probs_with_weights(
